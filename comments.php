...
if ( have_comments() || ( (!is_attachment() && comments_open() ) || ( is_attachment() && in_array( 'attachment_comments', $extra_form_settings ) ) ) ) {
	?>
	<div class="comments-container clearfix"><?php
		if ( have_comments() ) {
			...
			if ( $comments_count_only ) {
				...
				<ol class="commentlist"><?php
					$args = ( in_array( 'comment_separate', $extra_form_settings ) ) ? 'callback=rtp_comment_list&type=comment' : 'callback=rtp_comment_list&type=all';
		...
		<?php } ?>
			...
			<!-- Comment Pagination --><?php if ( get_previous_comments_link() || get_next_comments_link() ) { ?>
		...
		if ( in_array( 'comment_separate', $extra_form_settings ) && get_comments_number() ) {
			?>
