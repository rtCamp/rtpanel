...
function rtp_no_ellipsis( $text ) {
	global $post;
	$alignment = ' right';
	if ( rtp_get_titan_option( 'summary_show' ) && rtp_get_titan_option( 'thumbnail_show' ) && ( 'right' == rtp_get_titan_option( 'thumbnail_position' ) ) ) {
		$alignment = ' left';
	}
	$read_text = ( rtp_get_titan_option( 'read_text' ) ) ? rtp_get_titan_option( 'read_text' ) : '';
...
function rtp_new_excerpt_length( $length ) {
	$word_limit = rtp_get_titan_option( 'word_limit' );
	if ( ! empty( $word_limit ) ) {
		if ( preg_match( '/^[0-9]{1,3}$/i', $word_limit ) ) {
			$length = $word_limit;
		}
	} else {
		$length = 55;
...
function rtp_show_post_thumbnail( $post_id = null, $thumbnail_size = 'thumbnail', $default_img_path = '' ) {
	...
	if ( 'none' == rtp_get_titan_option( 'thumbnail_position' ) ) {
		$image_align = 'alignnone';
	} elseif ( 'left' == rtp_get_titan_option( 'thumbnail_position' ) ) {
		$image_align = 'alignleft';
	} elseif ( 'center' == rtp_get_titan_option( 'thumbnail_position' ) ) {
		$image_align = 'aligncenter';
	} elseif ( 'right' == rtp_get_titan_option( 'thumbnail_position' ) ) {
		$image_align = 'alignright';
	}

	if ( !is_singular() && rtp_get_titan_option( 'summary_show' ) && rtp_get_titan_option( 'thumbnail_show' ) && !rtp_is_bbPress() ) {
		
		if ( has_post_thumbnail() ) {
			echo '<figure class="rtp-thumbnail-container ' . esc_attr( $image_align ) . '">';
			?>
			<a role="link" class="<?php echo esc_attr( $image_align ); ?>" href="<?php echo get_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_post_thumbnail( $thumbnail_size, array( 'title' => the_title_attribute( array( 'echo' => false ) ), 'class' => 'post-thumb ' . $image_align ) ); ?></a><?php
			echo '</figure>';
		} else {
			...
			if ( isset( $image ) && !empty( $image ) ) {
				...
				echo '<figure class="rtp-thumbnail-container ' . esc_attr( $image_align ) . '">';
				?>
				<a role="link" class="<?php echo esc_attr( $image_align ); ?>" href="<?php echo get_permalink(); ?>" title="<?php the_title_attribute(); ?>"><img role="img" class="<?php echo 'post-thumb ' . esc_attr( $image_align ); ?> wp-post-image" title="<?php the_title_attribute(); ?>" alt="<?php echo esc_attr( $alt ); ?>" src="<?php echo esc_attr( $image ); ?>" /></a><?php
				echo '</figure>';
			}
		}
	}
