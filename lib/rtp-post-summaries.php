...
function rtp_no_ellipsis( $text ) {
    ...
    $text .= ( esc_attr( $read_text ) ) ? '<a class="rtp-readmore" title="' . sprintf( __( 'Read More On %s', 'rtPanel' ), get_the_title() ) . '" href="' . get_permalink( $post->ID ) . '" rel="nofollow">' . esc_attr( $read_text ) . '</a>' : '';
    return $text;
}
